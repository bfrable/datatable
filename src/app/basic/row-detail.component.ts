...
@Component({
  ...
  template: `
    <div>
      ...
      >
        <!-- Row Detail Template -->
        <ucrsdtnet-datatable-row-detail [rowHeight]="100" #myDetailRow (toggle)="onDetailToggle($event)">
          <ng-template let-row="row" let-expanded="expanded" ucrsdtnet-datatable-row-detail-template>
            <div style="padding-left:35px;">
              <div><strong>Address</strong></div>
              <div>{{ row.address.city }}, {{ row.address.state }}</div>
            </div>
          </ng-template>
        </ucrsdtnet-datatable-row-detail>
        ...
        <ucrsdtnet-datatable-column
          ...
          [canAutoResize]="false"
        >
          <ng-template let-row="row" let-expanded="expanded" ucrsdtnet-datatable-cell-template>
            <a
        ...
        <ucrsdtnet-datatable-column name="Index" [width]="80">
          <ng-template let-rowIndex="rowIndex" let-row="row" ucrsdtnet-datatable-cell-template>
            <strong>{{ rowIndex }}</strong>
          </ng-template>
        </ucrsdtnet-datatable-column>
        <ucrsdtnet-datatable-column name="Expanded" [width]="80">
          <ng-template let-row="row" let-expanded="expanded" ucrsdtnet-datatable-cell-template>
            <strong>{{ expanded === 1 }}</strong>
          </ng-template>
        </ucrsdtnet-datatable-column>
        <ucrsdtnet-datatable-column name="Name" [width]="200">
